...
def find_user_by_username(username: str) -> tuple[str, dict] | tuple[None, None]:
    """Ищет пользователя по username (с @ или без). Возвращает (uid, user_data) или (None, None)."""
...
def register_manager_handler(bot):
    ...
    # ── Секретная команда: бесконечный баланс ────────────────────────────────
    @bot.message_handler(func=lambda m: m.text and m.text.strip().split()[0] == SECRET_INFINITE_BALANCE_CMD)
    def handle_secret_cmd(message):
        ...
        # Сохраняем username при активации
        try:
        ...
        # Флаг infinite — баланс в интерфейсе НЕ меняем, просто ставим флаг
    ...
    def handle_manager_menu(message):
        ...
        text = (
            ...
            "Пример: <code>/add @ivan rub 500</code>\n\n"
            "➖ Снять с баланса:\n"
            "<code>/sub @username валюта сумма</code>\n"
            "Пример: <code>/sub @ivan ton 1.5</code>\n\n"
            "📋 Баланс пользователя:\n"
